...
#+begin_src emacs-lisp
  ...
  ;; use maggit on git bare repos like dotfiles repos
...
#+begin_src emacs-lisp
  (setq org-todo-keywords '(
                            (sequence "TODO(t)" "ACTIVE(a)" "NEXT(n)" "|" "DONE(d!)")
                            (sequence "BACKLOG(b)" "PLAN(p)" "TO_SEE(s)" "WAIT(w@/!)" "HOLD(h)" "|" "CANCELLED(k@)")
                            ))
  ;; )
#+end_src
...
#+begin_src emacs-lisp
  (setq org-capture-templates `(
                                ...
                                ("tt" "Task" entry 
                                 ...
                                 :empty-lines 1)
                                ("tb" "Bookmarks" entry 
                                 (file+olp+datetree "~/Org/Bookmarks.org")
                                 "* TO_SEE %?\n  %U\n  \n  %i" 
                                 :empty-lines 1)
