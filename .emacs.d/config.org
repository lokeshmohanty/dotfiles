...
#+begin_src emacs-lisp
  ...
  (leader-keys
    ...
    ;; "s m"   '(swiper-mc :which-key "swiper with multiple cursors")     ;; multiple-cursors package required
    "s b"   '(swiper-all :which-key "swiper in all buffers")
    "s g"   '(counsel-rg :which-key "grep in current directory")
  )
